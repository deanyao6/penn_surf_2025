...
# 24 months — brain age
ba24predictor = TabularPredictor.load(ba24)

eval = ba24predictor.evaluate(test)
rmse = -eval['root_mean_squared_error']
y_true = test['p24'].astype(float)
y_pred = ba24predictor.predict(test).astype(float)
rmse = np.sqrt(((y_pred - y_true)**2).mean())
mae  = np.abs(y_pred - y_true).mean()
print(f"BA@24 distillation — RMSE {rmse:.2f} mo | MAE {mae:.2f} mo")
...
is_extra = (test['bp24'].astype(float) == 1)
is_interp = ~is_extra

# plotting
plt.figure(figsize=(8, 6), dpi=200)
diagnoses = sorted(test['d24'].dropna().unique())

for diag in diagnoses:
    mask_diag = (test['d24'] == diag)
...
plt.xlabel('Predicted Brain Age at 24 Given MRI')
plt.ylabel('Predicted Brain Age at 24 Given Historical Progression and Baseline Scan')
plt.title('Brain Age at 24 months')
