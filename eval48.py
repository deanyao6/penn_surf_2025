...
# 48 months — brain age
ba48predictor = TabularPredictor.load(ba48)

eval = ba48predictor.evaluate(test)
rmse = -eval['root_mean_squared_error']
y_true = test['p48'].astype(float)
y_pred = ba48predictor.predict(test).astype(float)
rmse = np.sqrt(((y_pred - y_true)**2).mean())
mae  = np.abs(y_pred - y_true).mean()
print(f"BA@48 distillation — RMSE {rmse:.2f} mo | MAE {mae:.2f} mo")
...
is_extra = (test['bp48'].astype(float) == 1)
is_interp = ~is_extra

# plotting
plt.figure(figsize=(8, 6))
diagnoses = sorted(test['d48'].dropna().unique())

for diag in diagnoses:
    mask_diag = (test['d48'] == diag)
...
plt.xlabel('Predicted Brain Age at 48 Given MRI')
plt.ylabel('Predicted Brain Age at 48 Given Historical Progression and Baseline Scan')
plt.title('Brain Age at 48 months')
